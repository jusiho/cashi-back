...
export default factories.createCoreController('api::recurring-expense.recurring-expense' as any, ({ strapi }) => ({
  async find(ctx) {
  ...
  async update(ctx) {
    const existing = await strapi.service('api::recurring-expense.recurring-expense' as any).findOne(ctx.params.id, { populate: ['user'] });
