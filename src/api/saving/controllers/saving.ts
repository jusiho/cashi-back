...
export default factories.createCoreController('api::saving.saving' as any, ({ strapi }) => ({
  async find(ctx) {
  ...
  async update(ctx) {
    const existing = await strapi.service('api::saving.saving' as any).findOne(ctx.params.id, { populate: ['user'] });
